...
$(function() {
    ...
    $("#sliderCanvasZoom").slider({
        ...
        slide: function(event, ui) {
           ...
           $("#sliderDisplay").html(scale_val + "%");
            ...
            canvas_scale = scale;
            $canvas.width(origin_canvas_width * scale);
            $canvas.height(origin_canvas_height * scale);
            let image_divs = $("#canvas-background").find('> div');
            $("#canvas-background").find('> div').each(function(index){
    ...
    /*	
    	$( "#sliderLayerResize" ).slider({
    		range: "max",
    		min: 10,
    		max: 300,
    		value: 100,
    		slide: function( event, ui ) {
    			$( "#resizeLayer" ).val( ui.value );
    			if ($activeLayer) {
    				var scale = ui.value/100;
    				$activeLayer.find('img').width(Math.round($activeLayer.width()*scale));
    				$activeLayer.find('img').height(Math.round($activeLayer.height() * scale));
    			}
    		}
    	});	

    	$( "#resizeLayer" ).on('change', function(){
    		$( "#sliderLayerResize" ).slider( "option", "value", $(this).val());
    		if ($activeLayer) {
    			var scale = $(this).val()/100;
    			$activeLayer.width(Math.round($activeLayer.width() * scale));
    			$activeLayer.height(Math.round($activeLayer.height() * scale));
    		}
    	});
    */
...
let resize_width, resize_height;
    ...
    $('#fullSizeBtn').on('click', function(e) {
        ...
        html2canvas(document.getElementById('canvas-background'), { allowTaint: true, useCORS: true }).then(function(canvas) {
            ...
            debugBase64(canvas.toDataURL('image/' + $('input[name=fileExt]:checked').val()),$canvas.width(), $canvas.height());
